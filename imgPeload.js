let imgPreload = function(srcs, opts) {
  ...
  self.opts.method === 'unorder' && self.unorder()
  self.opts.method === 'order' && self.order()
}

imgPreload.defaultOpts = {
  // 加载的方式，默认为无序加载(unorder)：(即消耗内存小的会先加载)
  // 有序加载(order)：(按照img的顺序加载)
  method: 'unorder',
